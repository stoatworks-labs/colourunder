...
/**
    ...
    **A shader that will not compile.** `InitGL` returns `FF_FAIL` and from
    ...
    message anywhere; with seven passes this also records *which* one, which is
    ...
    **A pass buffer that could not be allocated.** This plugin holds a handful
    of line-raster buffers (at most 1024 samples by the standard's 576 or 480
    lines, RGBA16F) and the failure mode when the driver says no is a black
    frame with nothing to explain it. What goes in the log is the size asked
    for.
*/
namespace colourunder::diag
{
