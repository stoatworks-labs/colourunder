...
card and reports any that made no difference at all.
    ...
    python3 tools/sweep.py [--binary build/cutest] [--size WxH] [--jobs N]

Exit code 1 means something is dead.

------------------------------------------------------------------ the traps

**Every render is an average.** The sweep compares the MEAN of every frame
rendered (`cutest --average`), not the last one. The tracking error drifts and
the dropouts and the noise change with every video frame, so the last frame
alone can miss what a control did.

**The card moves**, so the time-base and the chroma delay have edges to act on.

**DOC needs dropouts to compensate**: its CONTEXT turns Wear up. With no
dropout on the tape DOC has nothing to do, which is correct, not dead.

**Tracking needs to reach the picture**: at 0 the bar rests in the vertical
interval, and Tracking 1 walks it through; 90 frames (1.5 s) see it.

**An option's range is its element count.** `cutest --list` prints it.
...
def render(binary, path, overrides):
    ...
    args = [binary, "--out", path, "--average", "--size", f"{WIDTH}x{HEIGHT}", "--frames", str(frames)]
...
def main():
    ...
    ap.add_argument("--binary", default=str(ROOT / "build" / "cutest"))
    ...
    scratch = tempfile.mkdtemp(prefix="cusweep")
